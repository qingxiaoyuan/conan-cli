  ...
  $('cat-hint').textContent =
    ...
    (all.length
      ? '共 ' +
        rows.length +
        ' 个组件' +
        (binTotal ? '，' + binTotal + ' 条制品。点组件展开查看平台 / 编译器 / Qt / 构建。' : '。点组件展开查看版本。')
      : '还没有结果，点查询。');
  if (missing && !err) $('cat-hint').textContent += ' 未能读取制品平台信息，请确认已登录远程仓库。';
  if (!all.length) {
    $('cat-list').innerHTML =
      `<div class="hint" style="padding:16px">${esc(err || '仓库里还没有组件，或还没查询。')}</div>`;
    return;
  }
  if (!rows.length) {
    $('cat-list').innerHTML =
      `<div class="hint" style="padding:16px">${esc(err || '没有匹配的组件，试试改筛选条件。')}</div>`;
    return;
  }
  $('cat-list').innerHTML = rows
    .map((row) => {
      ...
      const detailRows = row.bins.length
        ? row.bins
            .map((b) => {
              ...
              return `<tr><td class="mono">${esc(rec.version || '-')}</td><td>${esc(c.plat)}</td><td>${esc(c.comp)}</td><td>${esc(c.qt)}</td><td>${esc(c.bt)}</td><td><button type="button" class="btn" data-add="${esc(rec.reference)}">加入依赖</button></td></tr>`;
            })
            .join('')
        : row.versions
            .map(
              (ver) =>
                `<tr><td class="mono">${esc(ver)}</td><td>-</td><td>-</td><td>-</td><td>-</td><td><button type="button" class="btn" data-add="${esc(name + '/' + ver)}">加入依赖</button></td></tr>`
            )
            .join('') || '<tr><td colspan="6" class="detail">没有版本信息</td></tr>';
      const detail = open
        ? `<div class="vers"><table><thead><tr><th>版本</th><th>平台</th><th>编译器</th><th>Qt</th><th>构建</th><th>操作</th></tr></thead><tbody>${detailRows}</tbody></table></div>`
        : '';
      return `<div class="pkg-block${open ? ' open' : ''}"><button type="button" class="pkg${open ? ' open' : ''}" data-pkg="${esc(name)}"><span class="pkg-main"><span class="chev" aria-hidden="true">${open ? '▾' : '▸'}</span><span class="n">${esc(name)}</span></span><span class="c">${esc(summary)}</span></button>${detail}</div>`;
    })
    .join('');
  $('cat-list')
...
function bindCatalogEvents() {
  $('cat-q').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') act('catalog');
  });
